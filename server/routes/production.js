...
router.post('/formulas/:id/optimize', authenticate, async (req, res) => {
    try {
        const formula = queryOne('SELECT * FROM formulas WHERE id = ?', [req.params.id]);
        const ingredients = query('SELECT * FROM formula_ingredients WHERE formula_id = ?', [req.params.id]);
        ...
        for (const ing of ingredients) {
            const material = queryOne('SELECT * FROM raw_materials WHERE id = ?', [ing.raw_material_id]);
            const estimatedRate = material?.opening_rate || 10;
            const cost = (ing.percentage / 100) * 1000 * estimatedRate;
            totalCost += cost;
        ...
        res.json({
            ...
            data: {
                ...
                total_cost_per_1000kg: totalCost,
...
router.post('/batches/:id/complete', authenticate, requirePermission('production', 'edit'), async (req, res) => {
    try {
        ...
        const batch = queryOne('SELECT * FROM production_batches WHERE id = ?', [req.params.id]);
        if (!batch) {
            return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Batch not found' } });
        }
        ...
        if (productGodown && actual_qty > 0) {
            const productRate = queryOne('SELECT opening_rate FROM products WHERE id = ?', [batch.product_id]);

            const prevBalance = queryOne(`
                SELECT balance_qty, balance_amount FROM stock_ledger
                WHERE item_type = 'product' AND item_id = ? AND godown_id = ?
                ORDER BY created_at DESC LIMIT 1
            `, [batch.product_id, productGodown.id]);
...
router.put('/machines/:id', authenticate, requirePermission('production', 'edit'), async (req, res) => {
    try {
        ...
        const existing = queryOne('SELECT * FROM machines WHERE id = ?', [req.params.id]);
        if (!existing) {
        ...
        run(`UPDATE machines SET name = ?, type = ?, brand = ?, model = ?, capacity = ?, unit = ?, factory_id = ?, status = ?, last_maintenance = ?, next_maintenance = ?, is_active = ?
            ...
            [name, type, brand, model, capacity, unit, factory_id || req.factoryId, status || existing.status, 
             last_maintenance, next_maintenance, is_active ?? 1, req.params.id]);
...
router.delete('/machines/:id', authenticate, requirePermission('production', 'delete'), async (req, res) => {
    try {
        const inUse = queryOne('SELECT id FROM production_batches WHERE machine_id = ? AND status IN (?)', 
            [req.params.id, 'in_progress,planned']);
        if (inUse) {
            return res.status(400).json({ success: false, error: { code: 'CANNOT_DELETE', message: 'Machine has active batches' } });
        }

        run('UPDATE machines SET is_active = 0 WHERE id = ?', [req.params.id]);
...
router.post('/formulas/:id/optimize', authenticate, requirePermission('production', 'edit'), async (req, res) => {
    try {
        ...
        const items = query(`
            SELECT fi.*, rm.name as material_name, rm.opening_rate as price,
        ...
        const optimizedItems = optimizeFormula(items, target_qty, constraints);
        ...
        res.json({
            ...
            data: {
                ...
                optimized_formula: {
                    ...
                    total_cost: optimizedItems.reduce((sum, i) => sum + i.cost, 0),
...
function optimizeFormula(items, targetQty, constraints) {
    const optimized = items.map(item => {
        const price = item.price || 0;
        const currentQty = item.quantity;
        const ratio = currentQty / items.reduce((s, i) => s + i.quantity, 0);
        const optimizedQty = targetQty * ratio;
        ...
        return {
            ...
            price,
            cost: Math.round(optimizedQty * price * 100) / 100,
...
router.get('/formulas/:id/nutrients', authenticate, async (req, res) => {
    try {
        ...
        items.forEach(item => {
            const ratio = item.quantity / totalQty;
